...
func TestReceive_Success(t *testing.T) {
	...
	sinkCh := make(chan *common.FifoQueueItem, 1)
...
func TestReceive_AlreadyReceiving(t *testing.T) {
	...
	sinkCh1 := make(chan *common.FifoQueueItem)
	go func() {
	...
	// Try second receive
	sinkCh2 := make(chan *common.FifoQueueItem)
...
func TestReceiveHandler_MessageConversion(t *testing.T) {
	...
	var receivedItem *common.FifoQueueItem
	mockSub.receiveFunc = func(ctx context.Context, f func(context.Context, *pubsub.Message)) error {
		msg := &pubsub.Message{
			ID:          "msg-456",
...
func TestTrySend_Success(t *testing.T) {
	...
	item := &common.FifoQueueItem{MessageID: "test"}
	sinkCh := make(chan *common.FifoQueueItem, 1)
...
func TestTrySend_ContextCanceled(t *testing.T) {
	...
	item := &common.FifoQueueItem{MessageID: "test"}
	sinkCh := make(chan *common.FifoQueueItem) // unbuffered, will block
